...
먼저 펼쳐진 help를 보고, 내 실제 vault를 건드리지 않는 내장 demo smoke test를 실행합니다.
이 경로는 API key가 필요 없습니다.
...
명령 경로가 검증되면 운영자가 정한 vault만 명시적으로 등록합니다.
...
```bash
mindfresh onboard
# key/model 검증에서 실패했다면 수정 후 실패 지점부터 이어갑니다.
...
큰 주제 폴더는 원문 보존 sidecar도 생성할 수 있습니다. 기본
`--preserve-mode auto`에서는 파일 수가 많거나 원문 텍스트가 길어 단일
생성 파일로 보기 어려운 경우 `_generated/mindfresh/CONTEXT-*.md`를 씁니다.
이 파일들은 요약본이 아니라 source path와 hash가 포함된 순서 보존 Markdown
청크입니다. 따라서 서로 겹치지 않는 원문 맥락이 짧은 요약으로 사라지지
않습니다.
