...
def test_scanner_excludes_generated_internal_hidden_and_review_files(tmp_path: Path) -> None:
    ...
    topic_a = fixture.topic_a
    (topic_a / "SUMMARY.md").write_text("# generated summary\n", encoding="utf-8")
    (topic_a / "CHANGELOG.md").write_text("# generated changelog\n", encoding="utf-8")
    (topic_a / "generated-frontmatter.md").write_text(
        "---\nmindfresh_generated: true\n---\n# generated body\n", encoding="utf-8"
    )
    (fixture.root / ".mindfresh").mkdir()
    (fixture.root / ".mindfresh" / "manifest-note.md").write_text("# internal\n", encoding="utf-8")
    (topic_a / ".hidden").mkdir()
    (topic_a / ".hidden" / "secret.md").write_text("# hidden\n", encoding="utf-8")
    (topic_a / "_generated").mkdir()
    (topic_a / "_generated" / "draft.md").write_text("# generated\n", encoding="utf-8")
    (topic_a / "_review").mkdir()
    (topic_a / "_review" / "review.md").write_text("# review\n", encoding="utf-8")

    topics = detect_topics(fixture.root)
    topic_paths = [topic.relative_path.as_posix() for topic in topics]
    assert topic_paths == ["policy/compliance", "research/topic-b", "research/topic_a"]

    topic_a_topic = next(topic for topic in topics if topic.relative_path.as_posix() == "research/topic_a")
    source_names = [path.name for path in collect_topic_sources(topic_a_topic)]
    assert source_names == ["2026-04-01-baseline.md", "2026-04-20-comparison.md"]
    assert not is_source_markdown(topic_a / "generated-frontmatter.md")


def test_generated_outputs_are_never_reingested_after_creation(tmp_path: Path) -> None:
    fixture = create_fixture_vault(tmp_path)
    (fixture.topic_a / "SUMMARY.md").write_text(
        "---\nmindfresh_generated: true\nmindfresh_kind: summary\n---\n"
        "# Summary\n\nThis generated file contains rich markdown that must not be a source.\n",
        encoding="utf-8",
    )
    (fixture.topic_a / "CHANGELOG.md").write_text(
    ...
    topic_a_topic = next(
        ...
        if topic.relative_path.as_posix() == "research/topic_a"
    )

    assert {path.name for path in collect_topic_sources(topic_a_topic)} == {
